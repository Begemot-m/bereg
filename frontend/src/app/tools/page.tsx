...
function ClientTools() {
  ...
  return (
    <div>
      ...
      <Reveal y={10}>
        <div className="-mx-4 min-h-[64vh] rounded-t-[24px] px-4 pb-8 pt-5 @md:-mx-9 @md:px-9" style={{ background: "var(--surface)" }}>
          <section className="overflow-hidden rounded-[18px] bg-[var(--peach-soft)]" style={{ border: "var(--bw-lg) solid var(--peach-edge)" }}>
            ...
            <div className="flex items-center gap-2 border-t px-4 py-3" style={{ borderColor: "var(--peach-edge)" }}>
              <button onClick={() => { tap(); setTech(recommendation.tech); }} className="flex-1 rounded-[11px] bg-[var(--ink)] py-2.5 text-[13px] font-black text-white transition-transform active:scale-[.98]">Начать · {recommendation.time}</button>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            {CLIENT_PRACTICES.map((t, i) => {
              ...
              return <Reveal key={t.tech} delay={0.03 + i * 0.04}>
                <button onClick={() => { tap(); setTech(t.tech); }} className="group flex min-h-[226px] w-full flex-col overflow-hidden rounded-[17px] text-left transition-transform duration-200 active:scale-[.98]" style={{ background: t.bg, border: `var(--bw-lg) solid ${t.edge}` }}>
          ...
          {history.some((x) => typeof x.before === "number" && typeof x.after === "number") && <section className="mt-5 rounded-[15px] bg-white p-4" style={{ border: "var(--bw-lg) solid var(--edge-neutral)" }}>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <Link href="/therapy" onClick={() => tap()} className="rounded-[14px] bg-white p-3" style={{ border: "var(--bw) solid var(--green-edge)" }}><Icon name="mood" width={20} weight="bold" /><span className="mt-2 block text-[13px] font-black">Отметить настроение</span><span className="block text-[10px] font-semibold text-[var(--muted)]">быстрый чек-ин</span></Link>
            <Link href="/therapy" onClick={() => tap()} className="rounded-[14px] bg-white p-3" style={{ border: "var(--bw) solid var(--purple-edge)" }}><Icon name="balance" width={20} weight="bold" /><span className="mt-2 block text-[13px] font-black">Колесо баланса</span><span className="block text-[10px] font-semibold text-[var(--muted)]">сферы жизни</span></Link>
          </div>
