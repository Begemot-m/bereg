...
function TherapyDashboard({ therapists, next, bookings, therapy, onMood, onBoard, onGuideSeen, onWheel }: { therapists: ReturnType<typeof useMyTherapists>; next: MyBooking | null; bookings: MyBooking[]; therapy: TherapyState; onMood: (mood: number, emotions: string[]) => void; onBoard: (text: string) => void; onGuideSeen: () => void; onWheel: (answers: WheelAnswers) => void }) {
  ...
  return (
    <div className="-mx-4 -mt-2 @md:-mx-9">
      ...
      <header className="px-4 pb-16 pt-3 @md:px-9" style={{ background: "var(--page)" }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-[var(--purple)]"><Icon name="therapy" width={20} weight="fill" /></span>
      ...
      <main className="relative -mt-9 rounded-t-[24px] px-4 pb-8 pt-5 @md:px-9" style={{ background: "var(--surface)" }}>
...
function FindTherapistBlock() {
  return (
    <Link href="/catalog" onClick={tap} className="mt-4 flex items-center gap-3 rounded-[16px] bg-[#fffdf7] p-3.5 transition-transform active:scale-[0.99]" style={{ border: "2.5px dashed var(--purple-edge)" }}>
...
function TherapistCard({ name, next, bookings, defaultOpen, onRemove }: { name: string; next: MyBooking | null; bookings: MyBooking[]; defaultOpen?: boolean; onRemove?: () => void }) {
  ...
  return (
    <div className="relative mt-3 rounded-[16px] bg-[#fffdf7] p-3" style={{ border: "var(--bw-lg) solid var(--purple-edge)" }}>
      ...
      <Link href={href} onClick={tap} className="flex gap-3">
        {portrait ? (
          <div className="relative h-[104px] w-[88px] shrink-0 overflow-hidden rounded-[13px]" style={{ background: "var(--purple-soft)" }}>
            <Image src={portrait} alt={`Портрет: ${name}`} fill sizes="88px" className="object-cover" unoptimized={/^(data:|blob:)/i.test(portrait)} />
          </div>
        ) : (
          <div className="flex h-[104px] w-[88px] shrink-0 items-center justify-center rounded-[13px] bg-[var(--green)] text-[24px] font-black" style={{ border: "var(--bw-lg) solid var(--green-edge)" }}>{name.charAt(0)}</div>
        )}
