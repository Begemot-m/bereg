...
export default function ClientHomeworkPage() {
  ...
  return <div><Link href="/therapy" onClick={tap} className="back-link mb-3">Назад в терапию</Link><PageHead title="Задания" sub="Активные задания и история" icon="book" /><ClientHomeworkDetail homework={query.data ?? []} onChanged={() => void qc.invalidateQueries({ queryKey: ["my-homework"] })} /></div>;
}
