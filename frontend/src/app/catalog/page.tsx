    ...
    <div className="-mx-4 -mt-6 @md:-mx-9">
      <header className="px-4 pb-14 pt-8 @md:px-9" style={{ background: "var(--page)" }}>
        <div className="flex items-start justify-between gap-3">
          <div><p className="text-[10px] font-black uppercase tracking-[.14em]">Психологи платформы</p><div className="mt-1 flex items-center gap-2.5"><span className="flex h-11 w-11 items-center justify-center rounded-[11px] bg-white"><Icon name="compass" width={22} weight="bold" color="var(--edge)" /></span><h1 className="font-tight text-[31px] font-black leading-none">Каталог</h1></div><p className="font-tight mt-2 max-w-[250px] text-[12px] font-bold leading-snug text-[var(--muted)]">Специалисты, которые подойдут именно вам.</p></div>
          <button onClick={() => { tap(); setSurveyOpen(true); }} className="flex w-[92px] shrink-0 flex-col items-center gap-1 rounded-[13px] bg-white px-2 py-2.5" style={{ border: "var(--bw) solid var(--ink)" }} aria-label="Собрать персональную подборку">
      ...
      <main className="relative -mt-8 min-h-[72vh] rounded-t-[24px] bg-[#fffdf7] px-4 pb-9 pt-4 @md:px-9" >
...
function AllControls({ filters, setFilters, sort, setSort, activeFilters, openFilters }: { filters: CatalogFilters; setFilters: (filters: CatalogFilters) => void; sort: SortMode; setSort: (sort: SortMode) => void; activeFilters: number; openFilters: () => void }) {
  return <Reveal delay={.03}><div className="mt-4 space-y-2"><label className="flex items-center gap-2 rounded-[12px] bg-white px-3.5 py-2.5 stroke"><Icon name="compass" width={16} color="var(--muted)" /><input value={filters.query} onChange={(event) => setFilters({ ...filters, query: event.target.value })} placeholder="Имя, подход или запрос" className="min-w-0 flex-1 bg-transparent text-[13px] font-bold outline-none placeholder:font-semibold placeholder:text-[var(--muted-2)]" />{filters.query && <button onClick={() => setFilters({ ...filters, query: "" })} className="font-black text-[var(--muted)]" aria-label="Очистить поиск">×</button>}</label><div className="flex gap-2"><button onClick={openFilters} className="relative flex flex-1 items-center justify-center gap-1.5 rounded-[10px] bg-white px-3 py-2 text-[11px] font-black stroke"><Icon name="filter" width={15} weight="bold" /> Фильтры{activeFilters > 0 && <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-[var(--coral)] px-1 text-[10px] stroke">{activeFilters}</span>}</button><label className="flex flex-[1.35] items-center gap-1.5 rounded-[10px] bg-white px-3 py-2 stroke"><Icon name="sort" width={15} weight="bold" /><select value={sort} onChange={(event) => setSort(event.target.value as SortMode)} className="min-w-0 flex-1 bg-transparent text-[11px] font-black outline-none">{SORTS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></div></div></Reveal>;
}
...
function PsyCard({ psy, onOpen }: { psy: Psy; onOpen: () => void }) {
  ...
  return (
    <button onClick={onOpen} className="chunk w-full overflow-hidden rounded-[19px] p-0 text-left transition-transform duration-200 active:scale-[.99]">
      <div className="flex gap-3.5 p-4">
        <div className="relative h-[132px] w-[106px] shrink-0 overflow-hidden rounded-[14px]" style={{ background: "var(--head-soft)" }}>
...
function PsyDetailView({ psy, prefs, onBack }: { psy: Psy; prefs: CatalogPrefs; onBack: () => void }) {
  ...
  return <div>
    ...
    <div className="-mx-4 -mt-9 space-y-5 rounded-t-[24px] px-4 pb-10 pt-5 @md:-mx-9 @md:px-9" style={{ background: "var(--surface)" }}>
...
function BookingMini({ psyName, tone, onDone }: { psyName: string; tone: { bg: string; soft: string; edge: string }; onDone: () => void }) {
  ...
  return (
    <div className="overflow-hidden rounded-[18px]" style={{ background: tone.soft }}>
      <button onClick={() => { tap(); setOpen(!open); }} className="flex w-full items-center gap-3 p-4 text-left" aria-expanded={open}>
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[11px] bg-white">
...
function MethodList({ psy }: { psy: Psy }) {
  ...
  return (
    <Section title="Методы">
      ...
      <button onClick={() => { tap(); setShow((v) => !v); }} className="flex w-full items-center gap-2.5 rounded-[13px] p-3.5" style={{ background: tone.soft }} aria-expanded={show}>
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[8px] bg-white"><Icon name="therapy" width={15} weight="bold" color={tone.edge} /></span>
        ...
        {psy.methods.map((method) => {
          ...
          return (
            <motion.button
              ...
              className={`rounded-[14px] p-3.5 text-left ${expanded ? "col-span-2" : ""}`}
...
function LocationBlock({ psy, details }: { psy: Psy; details: string }) {
  ...
  if (psy.format === "online") {
    return <Section title="Формат и место"><div className="flex items-start gap-3 panel p-4"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[10px] bg-[var(--head-soft)] stroke"><Icon name="video" width={19} weight="bold" /></span><div><p className="text-[13px] font-black">{details}</p><p className="mt-1 text-[11px] font-semibold text-[var(--muted)]">Языки: {psy.languages.join(", ")}</p></div></div></Section>;
  }
  ...
  return (
    <Section title="Формат и место">
      <div className="overflow-hidden rounded-[16px] bg-white stroke-lg">
        <button onClick={() => { tap(); setOpen((value) => !value); }} className="block w-full text-left" aria-expanded={open}>
          ...
          <div className="flex items-start gap-3 p-4">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[var(--head-soft)] stroke"><Icon name="pin" width={17} weight="bold" /></span>
        ...
        <Disclosure open={open} zoom autoScroll={false}>
          <div className="line-top p-3">
            ...
            <div className="grid grid-cols-3 gap-2">{routes.map((route) => <a key={route.label} href={route.href} target="_blank" rel="noopener noreferrer" onClick={success} className="flex items-center justify-center rounded-[10px] bg-[var(--surface-2)] px-2 py-2.5 text-[10px] font-black stroke">{route.label}</a>)}</div>
          </div>
        </Disclosure>
      </div>
    </Section>
...
function TelegramPoster({ psy }: { psy: Psy }) {
  return (
    <div className="relative overflow-hidden rounded-[18px] p-5" style={{ background: "var(--head)" }}>
...
function PhotoGallery({ psy }: { psy: Psy }) {
  return <Section title="Фотографии"><div className="-mx-4 flex snap-x gap-2 overflow-x-auto px-4 pb-1">{psy.photos!.map((photo, index) => { const src = asset(photo); return <div key={`${photo.slice(0, 24)}-${index}`} className="relative h-[174px] w-[132px] shrink-0 snap-start overflow-hidden rounded-[14px] bg-white stroke-lg"><Image src={src} alt={`${psy.name}, фотография ${index + 1}`} fill sizes="132px" className="object-cover" unoptimized={isInlineImage(src)} /></div>; })}</div></Section>;
}
...
function RatingBlock({ psy, canRate }: { psy: Psy; canRate: boolean }) {
  ...
  return (
    <Section title="Рейтинг и оценка">
      <div className="overflow-hidden rounded-[18px] bg-[var(--amber-soft)]">
        ...
        <div className="border-t bg-white/55 p-4" style={{ borderColor: "var(--amber-edge)" }}>
          {canRate ? <><p className="mb-2 text-center text-[10px] font-black uppercase tracking-[.06em] text-[var(--muted)]">Ваша оценка после встречи</p><div className="flex justify-center gap-2">{[1,2,3,4,5].map((value) => <motion.button key={value} whileTap={{ scale: .78 }} animate={mine === value ? { scale: [1, 1.16, 1] } : { scale: 1 }} onClick={() => rate(value)} className="flex h-10 w-10 items-center justify-center rounded-[10px] bg-white stroke" aria-label={`Оценка ${value}`}><Icon name="star" width={21} weight={mine >= value ? "fill" : "regular"} color="var(--amber-edge)" /></motion.button>)}</div>{mine > 0 && <p className="mt-2 text-center text-[11px] font-black">Спасибо — оценку можно изменить</p>}</> : <p className="text-center text-[11px] font-semibold text-[var(--muted)]">Оценку можно оставить после состоявшейся сессии.</p>}
        </div>
      </div>
    </Section>
  );
