...
export function CareModule() {
  return (
    <div className="relative overflow-hidden rounded-[13px] p-5" style={{ background: "var(--purple)" }}>
      ...
      <div className="relative">
        <div className="flex items-center gap-2.5">
          <motion.span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[8px] text-[18px] font-black text-white" style={{ background: "var(--ink)" }} animate={{ rotate: [0, -6, 6, 0] }} transition={{ duration: 7, repeat: Infinity, ease: "easeInOut" }}>{APP_NAME.charAt(0)}</motion.span>
        ...
        <a href={TG_LINK} target="_blank" rel="noopener noreferrer" onClick={() => tap()} className="card-nested mt-4 flex items-center gap-3 p-3 transition-transform active:scale-[0.99]">
          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[8px]" style={{ background: "var(--purple-soft)" }}><Icon name="telegram" width={22} weight="fill" color="var(--purple-edge)" /></span>
