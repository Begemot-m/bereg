...
import { DailyDot, SectionTitle } from "@/components/blocks";
...
export function TodayCard({ items }: { items: TodayItem[] }) {
  return (
    <section>
      ...
      {/* Без рамки: это просто перечень того, что осталось сделать сегодня.
          Выполненное сюда не попадает — список собирается из невыполненного. */}
...
function TodayRow({ item, index }: { item: TodayItem; index: number }) {
  const inner = (
    <motion.div
      ...
      className="group flex items-start gap-3 py-1.5 transition-transform active:scale-[0.99]"
    >
      ...
      <DailyDot size={18} className="mt-0.5 shrink-0" label={`${item.title} — задание дня`} />
    </motion.div>
  );
