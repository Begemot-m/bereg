...
const WIDE_FROM = 448;
// Порог веб-версии — тот же, что в инлайн-скрипте `layout.tsx` и в демо.
const WEB_FROM = 768;
...
export function PhoneShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      } else if (window.innerWidth >= WEB_FROM) {
        document.documentElement.dataset.web = "1";
      } else {
        // Телефон в браузере: веб-версия рассчитана на широкий экран
        // (недельная сетка 840, таблица клиентов, дашборд в две колонки),
        // и на 390 её нечем показать — остаётся мобильная разметка.
        delete document.documentElement.dataset.web;
      }
    };
