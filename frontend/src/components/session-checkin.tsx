...
export function SessionCheckin({ bookings }: { bookings: MyBooking[] }) {
  ...
  return (
    <motion.section initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-3 rounded-[13px] bg-white p-4" style={{ border: "var(--bw-lg) solid var(--purple-edge)" }}>
      <AnimatePresence mode="wait">
        {done ? (
          <motion.div key="t" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2.5">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[7px] bg-[var(--green-soft)]" style={{ border: "var(--bw) solid var(--green-edge)" }}><Icon name="check" width={18} weight="bold" color="var(--green-edge)" /></span>
            <p className="text-[13px] font-black">Спасибо — это помогает вашей терапии</p>
          </motion.div>
        ) : (
          <motion.div key="q" exit={{ opacity: 0 }}>
            <div className="flex items-center gap-2.5">
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[7px] bg-[var(--purple-soft)]" style={{ border: "var(--bw) solid var(--purple-edge)" }}><Icon name="therapy" width={18} weight="bold" /></span>
            ...
            <div className="mt-3 grid grid-cols-3 gap-2">
              {OPTS.map((o) => (
                <button key={o.key} onClick={() => { tap(); answer(o.key); }} className="flex flex-col items-center gap-1.5 rounded-[8px] py-2.5 transition-transform active:scale-[0.96]" style={{ background: `var(--${o.tone}-soft)`, border: `var(--bw) solid var(--${o.tone}-edge)` }}>
