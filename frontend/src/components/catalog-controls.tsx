...
function StepFrame({ tone, icon, title, text, children }: { tone: string; icon: Parameters<typeof Icon>[0]["name"]; title: string; text: string; children?: React.ReactNode }) {
  return <div><div className="flex min-h-[118px] items-center justify-center rounded-[18px] stroke-lg" style={{ background: tone }}><div className="flex h-16 w-16 items-center justify-center rounded-[18px] bg-[#fffdf7] stroke"><Icon name={icon} width={31} weight="bold" /></div></div><h2 className="font-tight mt-4 text-[23px] font-black leading-[1.08]">{title}</h2><p className="mt-2 text-[13px] font-semibold leading-relaxed text-[var(--muted)]">{text}</p>{children && <div className="mt-4">{children}</div>}</div>;
}
