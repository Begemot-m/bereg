...
export function CatalogSurvey({ open, initial, catalog, onClose, onDone, onViewAll }: { open: boolean; initial: CatalogPrefs; catalog: Psy[]; onClose: () => void; onDone: (prefs: CatalogPrefs) => void; onViewAll: () => void }) {
  const [step, setStep] = useState(0);
  const [prefs, setPrefs] = useState(initial);
  useEffect(() => { if (open) { setPrefs(initial); setStep(0); } }, [open, initial]);
  const total = 7;
  const exact = useMemo(() => exactMatches(prefs, catalog).length, [prefs, catalog]);
  const selection = useMemo(() => personalSelection(prefs, catalog).length, [prefs, catalog]);
  ...
  return (
    <AnimatePresence>
      {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-end justify-center bg-[rgba(32,28,24,.46)] p-3 backdrop-blur-[2px] @md:items-center" onClick={onClose}>
        <motion.div initial={{ y: 34, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 34, opacity: 0 }} transition={{ type: "spring", stiffness: 410, damping: 32 }} onClick={(event) => event.stopPropagation()} className="chunk flex max-h-[min(88vh,calc(100dvh-var(--top-pad)))] w-full max-w-md flex-col overflow-hidden bg-[#ffffff]">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
            <motion.div key={step} initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .22 }}>
              ...
              {step === 2 && <FormatStep prefs={prefs} setPrefs={setPrefs} catalog={catalog} />}
              {step === 3 && <BudgetStep prefs={prefs} setPrefs={setPrefs} />}
              {step === 4 && <TimeStep prefs={prefs} setPrefs={setPrefs} catalog={catalog} />}
              {step === 5 && <PreferencesStep prefs={prefs} setPrefs={setPrefs} />}
              {step === 6 && <ResultStep exact={exact} />}
            </motion.div>
          </div>
...
function StepFrame({ tone, icon, title, text, children }: { tone: string; icon: Parameters<typeof Icon>[0]["name"]; title: string; text: string; children?: React.ReactNode }) {
  return <div><div className="flex min-h-[118px] items-center justify-center rounded-[18px] stroke-lg" style={{ background: tone }}><div className="flex h-16 w-16 items-center justify-center rounded-[18px] bg-[#ffffff] stroke"><Icon name={icon} width={31} weight="bold" /></div></div><h2 className="font-tight mt-4 text-[23px] font-black leading-[1.08]">{title}</h2><p className="mt-2 text-[13px] font-semibold leading-relaxed text-[var(--muted)]">{text}</p>{children && <div className="mt-4">{children}</div>}</div>;
}
function IntroStep() { return <StepFrame tone="var(--salmon-soft)" icon="compass" title="Найдём тех, с кем может сложиться" text="Ответьте на несколько вопросов, чтобы подобрать специалиста" />; }
function TopicsStep({ prefs, setPrefs }: { prefs: CatalogPrefs; setPrefs: (prefs: CatalogPrefs) => void }) { return <StepFrame tone="var(--amber-soft)" icon="heart" title="С чем хочется поработать?" text="Выберите до трёх тем или вернитесь к этому позже"><div className="flex flex-wrap gap-2">{SURVEY_TOPICS.map((topic) => <ToggleChip key={topic} active={prefs.topics.includes(topic)} onClick={() => setPrefs({ ...prefs, topics: toggleValue(prefs.topics, topic, 3) })}>{topic}</ToggleChip>)}</div></StepFrame>; }
function FormatStep({ prefs, setPrefs, catalog }: { prefs: CatalogPrefs; setPrefs: (prefs: CatalogPrefs) => void; catalog: Psy[] }) {
...
function TimeStep({ prefs, setPrefs, catalog }: { prefs: CatalogPrefs; setPrefs: (prefs: CatalogPrefs) => void; catalog: Psy[] }) {
  ...
  const fitting = useMemo(() => catalog.filter((psy) => availabilityFits(psyAvailability(psy), prefs.days, prefs.times)).length, [catalog, prefs.days, prefs.times]);
  return <StepFrame tone="var(--coral-soft)" icon="clock" title="Когда удобно встречаться?" text="Шаг можно пропустить. В подборке приоритет получат совпадающие окна."><p className="mb-2 text-[10px] font-black uppercase tracking-[.08em] text-[var(--muted)]">Дни</p><div className="grid grid-cols-2 gap-2"><ToggleChip active={prefs.days.includes("weekdays")} onClick={() => setPrefs({ ...prefs, days: toggleValue(prefs.days, "weekdays") })}>Будни</ToggleChip><ToggleChip active={prefs.days.includes("weekends")} onClick={() => setPrefs({ ...prefs, days: toggleValue(prefs.days, "weekends") })}>Выходные</ToggleChip></div><p className="mb-2 mt-4 text-[10px] font-black uppercase tracking-[.08em] text-[var(--muted)]">Время</p><div className="grid grid-cols-3 gap-2">{(["morning", "day", "evening"] as TimeOfDay[]).map((time) => <ToggleChip key={time} active={prefs.times.includes(time)} onClick={() => setPrefs({ ...prefs, times: toggleValue(prefs.times, time) })}>{TIME_LABEL[time]}</ToggleChip>)}</div>{(prefs.days.length > 0 || prefs.times.length > 0) && <p className="mt-3 text-[11px] font-bold text-[var(--muted)]">По графикам специалистов подходят {fitting} из {catalog.length}.</p>}</StepFrame>;
}
function PreferencesStep({ prefs, setPrefs }: { prefs: CatalogPrefs; setPrefs: (prefs: CatalogPrefs) => void }) { return <StepFrame tone="var(--sky)" icon="user" title="Есть важные пожелания?" text="Всё здесь необязательно и не влияет на качество специалиста."><p className="mb-2 text-[10px] font-black uppercase tracking-[.08em] text-[var(--muted)]">Пол специалиста</p><div className="grid grid-cols-3 gap-2">{(["any", "woman", "man"] as const).map((gender) => <ToggleChip key={gender} active={prefs.gender === gender} onClick={() => setPrefs({ ...prefs, gender })}>{gender === "any" ? "Неважно" : gender === "woman" ? "Женщина" : "Мужчина"}</ToggleChip>)}</div><p className="mb-2 mt-4 text-[10px] font-black uppercase tracking-[.08em] text-[var(--muted)]">Язык</p><div className="grid grid-cols-2 gap-2">{["any", ...SURVEY_LANGUAGES].map((language) => <ToggleChip key={language} active={prefs.language === language} onClick={() => setPrefs({ ...prefs, language })}>{language === "any" ? "Неважно" : language}</ToggleChip>)}</div></StepFrame>; }
function ResultStep({ exact }: { exact: number }) {
  return exact > 0
    ? <StepFrame tone="var(--green-soft)" icon="check" title={`Мы нашли ${exact} ${plural(exact, "специалиста", "специалистов", "специалистов")}`} text="Все они подходят под ваши ответы — посмотрите подборку." />
    : <StepFrame tone="var(--amber-soft)" icon="compass" title="Точных совпадений нет" text="Под ваши параметры есть похожие специалисты — покажем их в подборке." />;
}
