      ...
      {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-end justify-center bg-[rgba(32,28,24,.46)] p-3 backdrop-blur-[2px] @md:items-center" onClick={onClose}>
        <motion.div initial={{ y: 34, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 34, opacity: 0 }} transition={{ type: "spring", stiffness: 410, damping: 32 }} onClick={(event) => event.stopPropagation()} className="chunk flex max-h-[min(88vh,calc(100dvh-var(--top-pad)))] w-full max-w-md flex-col overflow-hidden bg-[#ffffff]">
          ...
          <div className="border-t px-5 py-4" style={{ borderColor: "var(--edge-neutral)" }}>
            {step === 0 ? <div className="space-y-2"><Button className="w-full" onClick={next}>Настроить подборку</Button><button onClick={onViewAll} className="w-full py-1 text-[12px] font-extrabold text-[var(--muted)]">Смотреть всех</button></div> : step === total - 1 ? <Button className="w-full" onClick={finish}>Посмотреть 6 специалистов</Button> : <div className="flex gap-2"><button className="back-link" onClick={prev}>Назад</button><Button className="flex-1" onClick={next}>{step === 4 || step === 5 ? "Далее" : "Продолжить"}</Button></div>}
          </div>
        </motion.div>
      </motion.div>}
    </AnimatePresence>
