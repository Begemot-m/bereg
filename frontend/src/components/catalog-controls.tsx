...
function ToggleChip({ active, children, onClick, className = "" }: { active: boolean; children: React.ReactNode; onClick: () => void; className?: string }) {
  return <button type="button" onClick={() => { select(); onClick(); }} className={`rounded-[10px] px-3 py-2 text-[12px] font-black transition-transform active:scale-95 stroke ${className}`} style={active ? { background: "var(--ink)", color: "#fff", borderColor: "var(--ink)" } : { background: "#fff" }}>{children}</button>;
}
  ...
  return (
    <AnimatePresence>
      {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] flex items-end justify-center bg-[rgba(32,28,24,.46)] p-3 backdrop-blur-[2px] @md:items-center" onClick={onClose}>
        <motion.div initial={{ y: 34, scale: .98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 34, opacity: 0 }} transition={{ type: "spring", stiffness: 410, damping: 32 }} onClick={(event) => event.stopPropagation()} className="chunk flex max-h-[min(88vh,calc(100dvh-var(--top-pad)))] w-full max-w-md flex-col overflow-hidden bg-[#fffdf7]">
...
function StepFrame({ tone, icon, title, text, children }: { tone: string; icon: Parameters<typeof Icon>[0]["name"]; title: string; text: string; children?: React.ReactNode }) {
  return <div><div className="flex min-h-[118px] items-center justify-center rounded-[16px] stroke-lg" style={{ background: tone }}><div className="flex h-16 w-16 items-center justify-center rounded-[16px] bg-[#fffdf7] stroke"><Icon name={icon} width={31} weight="bold" /></div></div><h2 className="font-tight mt-4 text-[23px] font-black leading-[1.08]">{title}</h2><p className="mt-2 text-[13px] font-semibold leading-relaxed text-[var(--muted)]">{text}</p>{children && <div className="mt-4">{children}</div>}</div>;
}
