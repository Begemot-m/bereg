    ...
    <div className="space-y-3.5">
      ...
      <div className="flex gap-1.5">
        {WEEKDAYS.map((label, wd) => {
          ...
          const isSel = day === wd;
          return (
            <button key={wd} onClick={() => { select(); setDay(wd); }} className="relative flex-1 rounded-[10px] py-1.5 text-[12px] font-extrabold transition-transform active:scale-95 stroke" style={isSel ? { background: "var(--ink)", color: "#fff", borderColor: "var(--ink)" } : { background: "#fff", color: cnt ? "var(--ink)" : "var(--muted-2)" }}>
              {label}
