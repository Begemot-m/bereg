...
export function WorkWithSpecialist({ sessionsDone, nextAt, homework, onChanged }: {
  ...
  return (
    <section data-tour="work" className="rounded-[13px] p-4" style={{ background: "var(--green-soft)" }}>
      ...
      <div className="mt-3 space-y-2">
        {homework.length === 0
          ? <p className="t-sub rounded-[10px] bg-white p-3">Заданий пока нет — терапевт пришлёт их после встречи.</p>
...
export function TherapistBoard({ value, onSave }: { value: string; onSave: (text: string) => void }) {
  ...
  return (
    <section data-tour="board" className="rounded-[13px] p-4" style={{ background: "var(--amber-soft)" }}>
      <div className="flex items-center justify-between">
        <div>
      ...
      <textarea
        ...
        className="t-body mt-3 w-full resize-none rounded-[10px] bg-white p-3.5 outline-none placeholder:font-normal placeholder:text-[var(--muted-2)]"
      />
