...
export function CatalogView({ psyId, book = false }: { psyId?: number; book?: boolean } = {}) {
  ...
  // Анкета открыта своим адресом (/catalog/anna-petrova-42) или ссылкой от
  // специалиста: /catalog?psy=<id>&book=1
  const [invited, setInvited] = useState(false);
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    setReturnTo(params.get("from"));
    const id = psyId ?? Number(params.get("psy"));
    if (!id) return;
    setInvited(book || params.get("book") === "1");
    ...
    // Пришли на адрес анкеты, а её нет (сняли с публикации, чужой id) — не
    // держим человека на пустой странице, возвращаем в каталог.
    const showFallback = () => { if (fallback) { setSelected(fallback); setSurveyOpen(false); } else if (psyId) router.replace("/catalog"); };
    ...
    return () => { alive = false; };
  }, [profile, work, me?.id, psyId, book, router]);
  ...
  useEffect(() => {
    try {
      ...
      const deep = psyId ?? new URLSearchParams(window.location.search).get("psy");
...
function PsyCard({ psy, index = 0, onOpen }: { psy: Psy; index?: number; onOpen: () => void }) {
  ...
  // У анкеты есть свой адрес — значит карточка обязана быть ссылкой: так её
  // видит поисковик и так работает «открыть в новой вкладке». Своя анкета
  // живёт только в браузере, для неё остаётся кнопка.
  const href = psy.id && psy.id !== OWN_PROFILE_ID ? psyPath(psy) : null;
  const className = "chunk w-full overflow-hidden rounded-[22px] p-0 text-left transition-transform duration-200 active:scale-[.99]";
  // Закрытый специалист приглушён: карточку можно открыть и прочитать,
  // но глазами она уже не зовёт записываться.
  const style = psy.accepting === false ? { opacity: 0.62 } : undefined;
  ...
  if (href) return <Link href={href} onClick={() => tap()} className={`block ${className}`} style={style}>{body}</Link>;
  return <button onClick={onOpen} className={className} style={style}>{body}</button>;
}
