...
const BFrame = ({ children }: { children: ReactNode }) => (
  <div className="flex min-h-[136px] flex-col justify-center gap-2 rounded-[13px] p-3" style={{ background: "var(--purple-soft)", border: "var(--bw) solid var(--purple-edge)" }}>{children}</div>
);
...
function FreeVsPro({ compact = false }: { compact?: boolean }) {
  return (
    <div className={`overflow-hidden rounded-[13px] bg-white ${compact ? "" : "stroke-lg"}`} style={compact ? { border: "var(--bw) solid var(--purple-edge)" } : undefined}>
...
export const PRO_BENEFITS: HelpPage[] = [
  { title: "Что бесплатно, а что в PRO", text: "Запись, график и первые карточки клиентов — бесплатно навсегда. PRO добавляет то, что экономит время на каждой сессии: статистику, сводку недели и шаблоны.", illo: (
    <div className="rounded-[13px] p-1" style={{ background: "var(--purple-soft)", border: "var(--bw) solid var(--purple-edge)" }}><FreeVsPro compact /></div>
  ) },
  { title: "Вся практика в одном месте", text: "Расписание, клиенты, записи и домашние задания рядом. Меньше рутины — больше времени на работу с людьми.", illo: (
    <BFrame>{["10:00 · Марина · онлайн", "15:00 · свободное окно", "19:00 · Алёна · очно"].map((t, i) => (
      <div key={t} className="flex items-center gap-2 rounded-[8px] bg-white px-2.5 py-1.5 text-[10px] font-bold" style={{ border: `var(--bw) solid ${["var(--purple-edge)", "var(--edge-neutral)", "var(--green-edge)"][i]}` }}>{t}</div>
    ))}</BFrame>
  ) },
  { title: "Новые инструменты каждый месяц", text: "Мы добавляем методики по научным подходам — колесо баланса, WHO-5, дневники мыслей. Всё уже включено в подписку.", illo: (
    <BFrame>{[["Колесо баланса", true], ["Дневник мыслей КПТ", true], ["Шкала тревоги GAD-7", false]].map(([t, isNew]) => (
      <div key={t as string} className="flex items-center gap-2 rounded-[8px] bg-white px-2.5 py-1.5" style={{ border: "var(--bw) solid var(--purple-edge)" }}>
  ...
  { title: "Профиль появляется в каталоге", text: "Размещение в каталоге — 500 ₽: подтверждённая анкета участвует в подборках на равных. Плата за размещение, а не за место — рейтинг и выдачу купить нельзя.", illo: (
    <BFrame>
      <div className="flex items-center gap-2 rounded-[8px] bg-[var(--green-soft)] px-2.5 py-2" style={{ border: "var(--bw) solid var(--green-edge)" }}>
        <span className="flex h-7 w-7 items-center justify-center rounded-[7px] bg-white" style={{ border: "1px solid var(--green-edge)" }}><Icon name="check" width={15} weight="bold" /></span>
      ...
      {["Совпадение с запросом", "Рейтинг после сессий"].map((t) => <div key={t} className="rounded-[8px] bg-white px-2.5 py-1.5 text-[10px] font-bold text-[var(--muted)]" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>{t}</div>)}
    </BFrame>
  ) },
];
...
export function SubscriptionBanner({ variant = "psy" }: { variant?: "psy" | "client" }) {
  ...
  return (
    <div className="overflow-hidden rounded-[18px]" style={{ background: "var(--purple-soft)" }}>
      <button onClick={() => { tap(); setOpen(!open); }} className="flex w-full items-center gap-3 p-4 text-left" aria-expanded={open}>
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-[12px]" style={{ background: "var(--purple)" }}>
        ...
        <div className="min-h-0 overflow-hidden">
          <div className="px-4 pb-4">
            ...
            <button
              ...
              className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-[11px] py-3 text-[13px] font-black text-white transition-transform active:scale-[0.98]"
...
export function SubscriptionBlock({ variant = "psy", compact = false }: { variant?: "psy" | "client"; compact?: boolean }) {
  ...
  return (
    <section className="overflow-hidden rounded-[19px]" style={{ border: "var(--bw-lg) solid var(--purple-edge)" }}>
      <div className="relative p-5" style={{ background: "linear-gradient(150deg, var(--purple) 0%, var(--purple-soft) 100%)" }}>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          {perks.map((p) => (
            <div key={p.label} className="flex items-center gap-2 rounded-[10px] bg-[#fffdf7] px-2.5 py-2" style={{ border: "var(--bw) solid var(--purple-edge)" }}>
              <Icon name={p.icon} width={15} weight="bold" /><span className="text-[11px] font-black leading-tight">{p.label}</span>
            </div>
          ))}
        </div>
        <button onClick={() => { tap(); setBenefits(true); }} className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-[11px] bg-[var(--ink)] py-2.5 text-[13px] font-black text-white">
...
function PlanCard({ plan, onPick, loading, defaultOpen = false }: { plan: Plan; onPick: () => void; loading: boolean; defaultOpen?: boolean }) {
  ...
  return (
    <div className="relative rounded-[14px]" style={{ background: best ? "var(--purple-soft)" : "#fff", border: `var(--bw-lg) solid ${best ? "var(--purple-edge)" : "var(--edge-neutral)"}` }}>
      ...
      <Disclosure open={open}>
        <div className="px-3.5 pb-3.5">
          ...
          <button onClick={() => { tap(); onPick(); }} disabled={loading} className="mt-3 w-full rounded-[10px] py-2.5 text-[13px] font-black transition-transform active:scale-[0.98] disabled:opacity-50" style={best ? { background: "var(--ink)", color: "#fff" } : { background: "#fff", border: "var(--bw) solid var(--purple-edge)" }}>
