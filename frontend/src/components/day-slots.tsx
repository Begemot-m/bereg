  ...
  return (
    <div className="space-y-1.5">
      ...
      {(hiddenPastCount > 0 || hasFutureSlots) && <div className="relative mb-1 flex items-center gap-2">
        ...
        {hasFutureSlots && menu && (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: -6, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={SPRING} className="absolute right-0 top-8 z-20 w-52 overflow-hidden rounded-[8px] p-1 stroke" style={{ background: "#fff", boxShadow: "0 12px 30px -12px rgba(32,28,24,.35)" }}>
      ...
      {visibleSlots.map((s) => {
        ...
        if (s.removed) {
          return (
            <motion.div key={s.iso} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 rounded-[7px] px-3 py-2 stroke-lg" style={{ background: "#f7f3ea", borderColor: "var(--edge-neutral)", opacity: 0.7 }}>
        ...
        if (s.past) {
          return (
            <motion.div key={s.iso} layout initial={{ opacity: 0 }} animate={{ opacity: 0.52 }} className="flex items-center gap-2 rounded-[7px] px-3 py-2 stroke-lg" style={{ background: st.bg, borderColor: st.bd }}>
        ...
        return (
          <motion.div key={s.iso} layout>
            <div className="flex items-center gap-2 rounded-[7px] px-3 py-2 stroke-lg" style={{ background: picking ? st.bg : st.bg, borderColor: st.bd }}>
...
function BusyRow({ appt, hour, onChanged }: { appt: Appointment; hour: number; onChanged: () => void }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: past ? 0.68 : 1, scale: 1 }} transition={SPRING} className="rounded-[7px] stroke-lg" style={{ background: "#fff", borderColor: "var(--edge-neutral)" }}>
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <button onClick={() => { tap(); setManage(!manage); }} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[6px]" style={manage ? { background: "var(--ink)", color: "#fff" } : { background: "var(--surface-2)" }} aria-label="Управлять сессией" aria-expanded={manage}><Icon name="gear" width={15} color={manage ? "#fff" : undefined} /></button>
      </div>
      <Disclosure open={manage}>
        <div className="px-3 pb-3">
          {resch ? (
            <div className="rounded-[7px] p-2.5" style={{ background: "#faf7f0" }}>
...
export function ClientPicker({ clients, onPick, compact = true, onCreateClient }: { clients: { id: number; name: string; status: string; contact?: string | null }[]; onPick: (id: number) => void; compact?: boolean; onCreateClient?: (name: string) => void }) {
  ...
  const row = (c: { id: number; name: string; status: string }) => (
    <button key={c.id} onClick={() => onPick(c.id)} className="flex w-full items-center gap-2 rounded-[6px] px-2 py-1.5 text-left transition-colors hover:bg-[var(--head-soft)] active:scale-[0.99]">
      <span className="flex h-7 w-7 items-center justify-center rounded-[6px] text-[12px] font-extrabold" style={{ background: c.status === "therapy" ? "var(--green-soft)" : "var(--head-soft)" }}>{c.name.charAt(0)}</span>
  ...
  return (
    <div className={`${compact ? "mt-1.5" : ""} rounded-[7px] p-2.5 stroke`} style={{ background: "#fff" }}>
      <div className="mb-1.5 flex items-center gap-2 rounded-[6px] bg-[var(--surface-2)] px-2.5 py-1.5" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>
      ...
      {onCreateClient && (adding ? (
        <form onSubmit={(e) => { e.preventDefault(); const n = newName.trim(); if (n) { onCreateClient(n); setNewName(""); setAdding(false); } }} className="mb-1.5 flex items-center gap-1.5">
          <input value={newName} onChange={(e) => setNewName(e.target.value)} autoFocus placeholder="Имя и фамилия" className="w-full rounded-[6px] bg-white px-2.5 py-1.5 text-[13px] font-semibold outline-none" style={{ border: "var(--bw) solid var(--olive-edge)" }} />
          <button type="submit" disabled={!newName.trim()} className="shrink-0 rounded-[6px] px-2.5 py-1.5 text-[12px] font-black text-white disabled:opacity-40" style={{ background: "var(--olive-edge)" }}>OK</button>
        ...
        <button onClick={() => { tap(); setAdding(true); }} className="mb-1.5 flex w-full items-center gap-2 rounded-[6px] px-2 py-1.5 text-left transition-colors active:scale-[0.99]" style={{ background: "var(--olive-soft)", border: "var(--bw) solid var(--olive-edge)" }}>
          <span className="flex h-7 w-7 items-center justify-center rounded-[6px] bg-white" style={{ border: "var(--bw) solid var(--olive-edge)" }}><Icon name="plus" width={14} weight="bold" color="var(--olive-edge)" /></span>
...
function MenuItem({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className="flex w-full items-center gap-2 rounded-[6px] px-2.5 py-2 text-left text-[13px] font-bold transition-colors hover:bg-[var(--head-soft)] active:scale-[0.99]">{children}</button>
  );
}
