...
function Intro({ tech, eyebrow, title, text, note, onStart, action = "Начать" }: { tech: TechKey; eyebrow: string; title: string; text: string; note: string; onStart: () => void; action?: string | null }) {
  ...
  return <div>
    ...
    <div className="mt-4 flex gap-2 rounded-[14px] bg-white p-3" style={{ border: "var(--bw) solid var(--edge-neutral)" }}><Icon name="spark" width={17} weight="bold" /><p className="text-[11px] font-semibold leading-relaxed text-[var(--muted)]">{note}</p></div>
...
function Primary({ onClick, disabled, children }: { onClick: () => void; disabled?: boolean; children: ReactNode }) {
  return <button onClick={() => { tap(); onClick(); }} disabled={disabled} className="mt-5 w-full rounded-[14px] bg-[var(--ink)] py-3.5 text-[14px] font-black text-white transition-transform duration-150 active:scale-[.98] disabled:opacity-40">{children}</button>;
}
...
function ThoughtRecord({ onProgress }: { onProgress: (n: number) => void }) {
  ...
  if (d.step < 0) return <div>
    ...
    <div className="mt-5 grid grid-cols-2 gap-2"><button onClick={() => upd({ mode: "quick", step: 0 })} className="rounded-[14px] bg-[var(--purple-soft)] p-3 text-left" style={{ border: "var(--bw) solid var(--purple-edge)" }}><b className="block text-[13px]">Новая запись · быстро</b><span className="text-[10px] font-semibold text-[var(--muted)]">4 коротких шага</span></button><button onClick={() => upd({ mode: "full", step: 0 })} className="rounded-[14px] bg-white p-3 text-left" style={{ border: "var(--bw) solid var(--edge-neutral)" }}><b className="block text-[13px]">Новая запись · подробно</b><span className="text-[10px] font-semibold text-[var(--muted)]">полная КПТ-запись</span></button></div>
...
function ThoughtJournalList({ journal, onOpen }: { journal: JournalEntry[]; onOpen: (e: JournalEntry) => void }) {
  if (!journal.length) return <div className="mt-6 rounded-[14px] bg-white p-4 text-center" style={{ border: "var(--bw) dashed var(--purple-edge)" }}><Icon name="book" width={22} weight="bold" /><p className="mt-1.5 text-[12px] font-black">История записей пуста</p><p className="mt-0.5 text-[11px] font-semibold text-[var(--muted)]">Первая запись появится здесь после сохранения.</p></div>;
  return <div className="mt-6">
    ...
    <div className="space-y-2">
      {journal.map((e) => {
        ...
        return <button key={e.id} onClick={() => { tap(); onOpen(e); }} className="flex w-full items-center gap-3 rounded-[14px] bg-white p-3 text-left transition-transform active:scale-[.99]" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>
