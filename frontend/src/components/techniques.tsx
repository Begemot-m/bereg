      ...
      <header className="shrink-0 px-4 pb-5 pt-[max(14px,var(--top-pad))]" style={{ background: c.bg, borderBottom: `var(--bw-lg) solid ${c.edge}` }}>
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2.5">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] bg-[#fffdf7]" style={{ border: `var(--bw) solid ${c.edge}` }}><Icon name={meta.icon} width={20} weight="bold" /></span>
...
function Intro({ tech, eyebrow, title, text, note, onStart, action = "Начать" }: { tech: TechKey; eyebrow: string; title: string; text: string; note: string; onStart: () => void; action?: string | null }) {
  ...
  return <div>
    ...
    <div className="mt-4 flex gap-2 rounded-[10px] bg-white p-3" style={{ border: "var(--bw) solid var(--edge-neutral)" }}><Icon name="spark" width={17} weight="bold" /><p className="text-[11px] font-semibold leading-relaxed text-[var(--muted)]">{note}</p></div>
...
function Primary({ onClick, disabled, children }: { onClick: () => void; disabled?: boolean; children: ReactNode }) {
  return <button onClick={() => { tap(); onClick(); }} disabled={disabled} className="mt-5 w-full rounded-[10px] bg-[var(--ink)] py-3.5 text-[14px] font-black text-white transition-transform duration-150 active:scale-[.98] disabled:opacity-40">{children}</button>;
}

function Back({ onClick }: { onClick: () => void }) {
  return <button onClick={() => { tap(); onClick(); }} className="mt-5 flex w-full items-center justify-center gap-1.5 rounded-[9px] bg-white py-3 text-[13px] font-black" style={{ border: "var(--bw) solid var(--edge-neutral)" }}><ArrowLeft size={16} weight="bold" /> Назад</button>;
}

function Rating({ value, onChange, tone, label }: { value: number; onChange: (v: number) => void; tone: Tone; label: string }) {
  const c = TONE[tone];
  return <div><div className="mb-2 flex items-end justify-between"><p className="text-[12px] font-black">{label}</p><span className="tnum rounded-full px-2.5 py-1 text-[13px] font-black" style={{ background: c.soft, border: `var(--bw) solid ${c.edge}` }}>{value}/10</span></div><div className="grid grid-cols-11 gap-1">{Array.from({ length: 11 }, (_, i) => <button key={i} onClick={() => { select(); onChange(i); }} className="h-9 rounded-[6px] text-[10px] font-black transition-transform active:scale-90" style={i <= value ? { background: c.bg, border: `var(--bw) solid ${c.edge}` } : { background: "#fff", border: "var(--bw) solid var(--edge-neutral)", color: "var(--muted-2)" }} aria-label={`${i} из 10`}>{i}</button>)}</div></div>;
}
...
function Breathing({ onProgress }: { onProgress: (n: number) => void }) {
  ...
  if (stage === "setup") return <div><h2 className="font-tight text-[23px] font-black">Настройте практику</h2><p className="mt-1 text-[12px] font-semibold text-[var(--muted)]">Начните с минуты. Остановиться можно в любой момент.</p>
    <p className="lbl mt-5">Ритм</p><div className="grid grid-cols-2 gap-2">{([{ id: "slow", title: "4 · 6", text: "вдох · выдох" }, { id: "478", title: "4 · 7 · 8", text: "с паузой" }] as const).map((x) => <button key={x.id} onClick={() => { select(); setProtocol(x.id); }} className="rounded-[11px] p-3 text-left" style={protocol === x.id ? { background: c.soft, border: `var(--bw-lg) solid ${c.edge}` } : { background: "#fff", border: "var(--bw) solid var(--edge-neutral)" }}><span className="block font-tight text-[20px] font-black">{x.title}</span><span className="text-[11px] font-semibold text-[var(--muted)]">{x.text}</span></button>)}</div>
    {protocol === "478" && <p className="mt-2 rounded-[8px] bg-[var(--amber-soft)] p-3 text-[11px] font-semibold text-[var(--muted)]" style={{ border: "var(--bw) solid var(--amber-edge)" }}>Пауза подходит не всем. Если появляется дискомфорт или головокружение, вернитесь к ритму 4 · 6.</p>}
    <p className="lbl mt-5">Длительность</p><div className="grid grid-cols-3 gap-2">{[1, 3, 5].map((m) => <button key={m} onClick={() => { select(); setMinutes(m); }} className="rounded-[8px] py-3 text-[13px] font-black" style={minutes === m ? { background: "var(--ink)", color: "#fff" } : { background: "#fff", border: "var(--bw) solid var(--edge-neutral)" }}>{m} мин</button>)}</div>
  ...
  if (stage === "run") { const current = phases[phase]; const pct = 1 - phaseLeft / current.seconds; return <div className="flex min-h-full flex-col items-center"><div className="flex w-full items-center justify-between"><span className="rounded-full bg-white px-3 py-1.5 text-[11px] font-black" style={{ border: `var(--bw) solid ${c.edge}` }}>{Math.floor(totalLeft / 60)}:{String(totalLeft % 60).padStart(2, "0")}</span><button onClick={() => { tap(); if (!sound) { const ctx = audioRef.current ?? new window.AudioContext(); audioRef.current = ctx; void ctx.resume().then(() => playBreathCue(ctx)); setSound(true); } else { setSound(false); } }} className="flex h-9 w-9 items-center justify-center rounded-full bg-white" style={{ border: `var(--bw) solid ${c.edge}` }} aria-label={sound ? "Выключить сигнал" : "Включить сигнал"}>{sound ? <SpeakerHigh size={17} weight="bold" /> : <SpeakerSlash size={17} weight="bold" />}</button></div>
    <div className="relative my-5 flex h-[300px] w-full items-center justify-center overflow-hidden rounded-[16px]" style={{ background: c.soft, border: `var(--bw-lg) solid ${c.edge}` }}>
    ...
    <p className="text-center text-[11px] font-semibold text-[var(--muted)]">Дышите комфортно. Не нужно наполнять лёгкие до предела.</p><button onClick={() => { tap(); setRunning(!running); }} className="mt-5 flex w-full items-center justify-center gap-2 rounded-[10px] bg-[var(--ink)] py-3.5 text-[14px] font-black text-white">{running ? <Pause size={18} weight="fill" /> : <Play size={18} weight="fill" />}{running ? "Пауза" : "Продолжить"}</button><button onClick={() => { setRunning(false); setStage("after"); }} className="mt-2 py-2 text-[11px] font-black text-[var(--muted)]">Завершить раньше</button></div>; }
...
function ThoughtRecord({ onProgress }: { onProgress: (n: number) => void }) {
  ...
  if (d.step < 0) return <div>
    ...
    <div className="mt-5 grid grid-cols-2 gap-2"><button onClick={() => upd({ mode: "quick", step: 0 })} className="rounded-[10px] bg-[var(--purple-soft)] p-3 text-left" style={{ border: "var(--bw) solid var(--purple-edge)" }}><b className="block text-[13px]">Новая запись · быстро</b><span className="text-[10px] font-semibold text-[var(--muted)]">4 коротких шага</span></button><button onClick={() => upd({ mode: "full", step: 0 })} className="rounded-[10px] bg-white p-3 text-left" style={{ border: "var(--bw) solid var(--edge-neutral)" }}><b className="block text-[13px]">Новая запись · подробно</b><span className="text-[10px] font-semibold text-[var(--muted)]">полная КПТ-запись</span></button></div>
...
function ThoughtJournalList({ journal, onOpen }: { journal: JournalEntry[]; onOpen: (e: JournalEntry) => void }) {
  if (!journal.length) return <div className="mt-6 rounded-[10px] bg-white p-4 text-center" style={{ border: "var(--bw) dashed var(--purple-edge)" }}><Icon name="book" width={22} weight="bold" /><p className="mt-1.5 text-[12px] font-black">История записей пуста</p><p className="mt-0.5 text-[11px] font-semibold text-[var(--muted)]">Первая запись появится здесь после сохранения.</p></div>;
  return <div className="mt-6">
    ...
    <div className="space-y-2">
      {journal.map((e) => {
        ...
        return <button key={e.id} onClick={() => { tap(); onOpen(e); }} className="flex w-full items-center gap-3 rounded-[10px] bg-white p-3 text-left transition-transform active:scale-[.99]" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>
...
function ThoughtEntryView({ entry, onBack, onDelete }: { entry: JournalEntry; onBack: () => void; onDelete: () => void }) {
  ...
  return <div>
    ...
    <div className="mt-4 space-y-4 rounded-[11px] bg-white p-4" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>
