...
export function ClientSelect({ value, onChange }: { value: number | null; onChange: (id: number) => void }) {
  ...
  const Avatar = ({ name, therapy }: { name: string; therapy: boolean }) => (
    <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-[7px] stroke text-[12px] font-extrabold" style={{ background: therapy ? "var(--green-soft)" : "var(--head-soft)" }}>{name.charAt(0)}</span>
  );

  return (
    <div>
      <button onClick={() => { select(); setOpen(!open); }} className="flex w-full items-center gap-2 rounded-[10px] px-2.5 py-2 stroke" style={{ background: "#fff" }}>
        {sel ? <Avatar name={sel.name} therapy={sel.status === "therapy"} /> : <span className="flex h-7 w-7 items-center justify-center rounded-[7px] stroke" style={{ background: "var(--head-soft)" }}>?</span>}
      ...
      <Disclosure open={open}>
        <div className="mt-1.5 rounded-[10px] p-2 stroke" style={{ background: "#fff" }}>
          ...
          <div className="no-scrollbar mt-1.5 flex max-h-44 flex-col gap-0.5 overflow-y-auto">
            {filtered.map((c) => (
              <button key={c.id} onClick={() => { select(); onChange(c.id); setOpen(false); setQ(""); }} className="flex items-center gap-2 rounded-[7px] px-2 py-1.5 text-left active:scale-[0.99]">
