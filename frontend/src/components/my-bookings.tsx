  ...
  return (
    <motion.div layout initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: past ? 0.68 : 1, scale: 1 }} transition={SPRING} className="card-nested overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <button onClick={toggle} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px]" style={manage ? { background: "var(--ink)", color: "#fff" } : { background: "var(--surface-2)" }} aria-label="Управлять записью" aria-expanded={manage}>
      ...
      <Disclosure open={manage}>
        <div className="px-3 pb-3">
          {resch ? (
            ...
            <div className="rounded-[14px] p-3" style={{ background: "var(--salmon-soft)" }}>
