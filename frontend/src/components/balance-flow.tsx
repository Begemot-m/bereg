...
// Экран результата: круглое колесо (радар), сильные/слабые сферы и (для клиента без Методика+) оплата.
    ...
    <div className="space-y-4">
      ...
      {locked && (
        <div className="rounded-[16px] p-4" style={{ background: "var(--purple-soft)", border: "var(--bw-lg) solid var(--purple-edge)" }}>
          ...
          <button onClick={() => { tap(); onUnlock?.(); }} className="mt-3 w-full rounded-[12px] bg-[var(--ink)] py-2.5 text-[13px] font-black text-white transition-transform active:scale-[0.98]">Открыть Методика+ · 390 ₽/мес</button>
        </div>
      )}
