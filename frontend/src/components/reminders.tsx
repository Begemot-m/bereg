...
export function RemindersModule() {
  ...
  return (
    <div className="space-y-4">
      ...
      <button onClick={() => { select(); save({ ...cfg, channel: !cfg.channel }); }} className="flex w-full items-center gap-2.5 rounded-[13px] bg-white p-3 text-left" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>
        <span className="flex h-8 w-8 items-center justify-center rounded-[9px]" style={{ background: "var(--head-soft)" }}><Icon name="telegram" width={16} weight="fill" /></span>
...
function Panel({ icon, title, hint, children }: { icon: "users" | "user"; title: string; hint: string; children: React.ReactNode }) {
  return (
    <div className="rounded-[14px] bg-[var(--surface-2)] p-3" style={{ border: "var(--bw) solid var(--edge-neutral)" }}>
      <div className="mb-2.5 flex items-center gap-2">
        <span className="flex h-8 w-8 items-center justify-center rounded-[9px] bg-white" style={{ border: "var(--bw) solid var(--edge-neutral)" }}><Icon name={icon} width={15} /></span>
