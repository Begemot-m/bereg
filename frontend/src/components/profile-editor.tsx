...
function ProfileProgress({ profile, onContinue }: { profile: PsyProfile | null; onContinue: () => void }) {
  ...
  return (
    <button onClick={onContinue} className="flex w-full items-center gap-3 rounded-[11px] p-3 text-left transition-transform active:scale-[.99]" style={{ background: `var(--${tone}-soft)`, border: `var(--bw-lg) solid var(--${tone}-edge)` }}>
      <motion.span
        className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[8px] bg-white stroke"
...
function CatalogThumb({ profile, name, photo }: { profile: PsyProfile | null; name: string; photo: string | null }) {
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-[14px] bg-white" style={{ border: "var(--bw-lg) solid var(--edge-neutral)", boxShadow: "0 16px 32px -22px rgba(32,28,24,.42)" }}>
        <div className="flex gap-3.5 p-4">
          <div className="relative flex h-[132px] w-[106px] shrink-0 items-center justify-center overflow-hidden rounded-[11px] text-[30px] font-black" style={{ border: "var(--bw-lg) solid var(--olive-edge)", background: "var(--olive-soft)" }}>
...
function PreviewStat({ label, value }: { label: string; value: string }) { return <div className="rounded-[8px] bg-[var(--surface-2)] p-2 text-center stroke"><p className="tnum truncate text-[12px] font-black">{value}</p><p className="mt-1 text-[8px] font-black uppercase tracking-[.04em] text-[var(--muted)]">{label}</p></div>; }
...
function SectionTitle({ icon, title }: { icon: IconName; title: string }) { return <div className="mb-2 flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-[6px] bg-[var(--head-soft)] stroke"><Icon name={icon} width={14} weight="bold" /></span><p className="text-[12px] font-extrabold text-[var(--muted)]">{title}</p></div>; }
function PreviewEmpty({ text }: { text: string }) { return <div className="rounded-[11px] bg-[var(--surface-2)] p-4 text-[13px] font-semibold text-[var(--muted)] stroke">{text}</div>; }
...
function ProfileForm({ onDone }: { onDone: () => void }) {
  ...
  return <div>
    ...
    <div className="mb-4 overflow-hidden rounded-[13px] bg-white stroke-lg">
      ...
      <div className="flex items-center gap-3 p-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-[8px] stroke" style={{ background: current.tone }}><Icon name={current.icon} width={19} weight="bold" /></span>
        <div className="min-w-0 flex-1">
    ...
    {error && <motion.p initial={{ opacity: 0, y: -3 }} animate={{ opacity: 1, y: 0 }} className="mt-3 rounded-[8px] bg-[var(--salmon-soft)] px-3 py-2 text-[12px] font-bold stroke" style={{ borderColor: "var(--salmon-edge)" }}>{error}</motion.p>}
...
function IdentityStep({ draft, update, fileRef }: { draft: PsyProfile; update: (patch: Partial<PsyProfile>) => void; fileRef: React.RefObject<HTMLInputElement | null> }) {
  ...
  return <StepCard title="Сначала — то, что помогает узнать вас" hint="Первое фото станет крупным портретом в каталоге. Можно добавить до трёх.">
    ...
    <Field label="Telegram для связи"><div className="flex items-center gap-2 rounded-[8px] bg-white px-3 stroke"><span className="font-black text-[var(--muted-2)]">@</span><input value={draft.tg} onChange={(event) => update({ tg: event.target.value.replace(/^@/, "") })} placeholder="username" className="w-full bg-transparent py-2.5 text-sm font-semibold outline-none" /></div></Field>
    ...
    <div className="space-y-2">
      {links.map((link, i) => (
        <div key={i} className="flex items-center gap-2">
          <select value={link.kind} onChange={(e) => setAt(i, { kind: e.target.value as LinkKind })} className="shrink-0 rounded-[7px] bg-white px-2 py-2.5 text-[12px] font-black stroke outline-none">
          ...
          <button onClick={() => onChange(links.filter((_, idx) => idx !== i))} className="flex h-[42px] w-10 shrink-0 items-center justify-center rounded-[7px] bg-white stroke" aria-label="Удалить">×</button>
        </div>
      ))}
      <button onClick={() => onChange([...links, { kind: "site", url: "" }])} className="flex w-full items-center justify-center gap-1.5 rounded-[7px] bg-white py-2 text-[13px] font-bold stroke"><Icon name="plus" width={15} /> Добавить ссылку</button>
    </div>
  );
}
...
function PublishedScreen({ name, onDone }: { name: string; onDone: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center py-6 text-center">
      <motion.span initial={{ scale: 0.6 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 260, damping: 16 }} className="flex h-20 w-20 items-center justify-center rounded-[16px] bg-[var(--amber)]" style={{ border: "var(--bw-lg) solid var(--amber-edge)" }}>
      ...
      <div className="mt-4 flex items-center gap-2 rounded-[8px] bg-[var(--amber-soft)] px-3.5 py-2.5 text-[11px] font-black" style={{ border: "var(--bw) solid var(--amber-edge)" }}><Icon name="check" width={13} weight="bold" /> Пока идёт проверка — профиль виден только вам</div>
...
function MethodsStep({ draft, update }: { draft: PsyProfile; update: (patch: Partial<PsyProfile>) => void }) {
  ...
  return <StepCard title="В каких подходах работаете" hint="Отметьте свои школы — можно несколько. Первый становится основным (рядом с именем), его сменить можно тапом по звёздочке.">
    ...
    <div className="flex items-center gap-2">
      <div className="flex flex-1 items-center gap-2 rounded-[8px] bg-white px-3 stroke"><Icon name="plus" width={15} color="var(--muted-2)" /><input value={custom} onChange={(event) => setCustom(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addCustom(); } }} placeholder="Свой подход" className="w-full bg-transparent py-2.5 text-[13px] font-semibold outline-none" /></div>
      <button onClick={addCustom} disabled={!custom.trim()} className="shrink-0 rounded-[8px] bg-[var(--ink)] px-4 py-2.5 text-[12px] font-black text-white disabled:opacity-40">Добавить</button>
    </div>
...
function FormatStep({ draft, update, updateLocation }: { draft: PsyProfile; update: (patch: Partial<PsyProfile>) => void; updateLocation: (patch: Partial<PsyProfile["location"]>) => void }) {
  ...
  return <StepCard title="Как проходят встречи" hint="Можно выбрать сразу оба формата. Точный адрес по умолчанию скрыт — вы сами решаете, показывать ли его.">
    ...
    {offline && <div className="space-y-3 rounded-[11px] bg-white p-3 stroke">
...
function FormatToggle({ active, icon, label, onClick }: { active: boolean; icon: IconName; label: string; onClick: () => void }) {
  return (
    <button onClick={() => { select(); onClick(); }} className="flex items-center justify-center gap-2 rounded-[9px] py-3 text-[13px] font-black transition-transform active:scale-[0.98]" style={{ background: active ? "var(--olive)" : "#fff", border: `var(--bw) solid var(--${active ? "olive-edge" : "edge-neutral"})`, color: "var(--ink)" }}>
      <Icon name={icon} width={17} weight="bold" /> {label}
    </button>
  );
}

function ConditionsStep({ draft, update }: { draft: PsyProfile; update: (patch: Partial<PsyProfile>) => void }) { return <StepCard title="Условия одной встречи" hint="Именно эти значения используются в фильтрах и на карточке каталога."><Field label="Стоимость, ₽"><Input type="number" min={0} step={100} value={draft.sessionPrice} onChange={(event) => update({ sessionPrice: Number(event.target.value) })} /></Field><Field label="Длительность"><div className="flex items-center gap-2.5"><button onClick={() => { select(); update({ sessionMinutes: Math.max(30, draft.sessionMinutes - 5) }); }} className="flex h-11 w-11 items-center justify-center rounded-[8px] bg-white text-[20px] font-black stroke" aria-label="Уменьшить">−</button><div className="flex h-11 min-w-[104px] items-center justify-center rounded-[8px] bg-[var(--head-soft)] px-3 stroke"><span className="tnum text-[16px] font-black">{draft.sessionMinutes} мин</span></div><button onClick={() => { select(); update({ sessionMinutes: Math.min(120, draft.sessionMinutes + 5) }); }} className="flex h-11 w-11 items-center justify-center rounded-[8px] bg-white text-[20px] font-black stroke" aria-label="Увеличить">+</button></div></Field><div className="rounded-[10px] bg-[var(--green-soft)] p-3 text-[11px] font-semibold leading-relaxed stroke" style={{ borderColor: "var(--green-edge)" }}>Размещение в каталоге платное, но цена размещения не влияет на рейтинг и порядок рекомендаций.</div></StepCard>; }
...
function ChipInput({ placeholder, onAdd }: { placeholder: string; onAdd: (value: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex flex-1 items-center gap-2 rounded-[8px] bg-white px-3 stroke"><Icon name="plus" width={15} color="var(--muted-2)" /><input value={value} onChange={(e) => setValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); commit(); } }} placeholder={placeholder} className="w-full bg-transparent py-2.5 text-[13px] font-semibold outline-none" /></div>
      <button onClick={commit} disabled={!value.trim()} className="shrink-0 rounded-[8px] bg-[var(--ink)] px-4 py-2.5 text-[12px] font-black text-white disabled:opacity-40">Добавить</button>
    </div>
  );
}
...
function BasicProfileForm({ onDone }: { onDone: () => void }) {
  ...
  return <div className="space-y-4"><Field label="Имя"><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="Как к вам обращаться" /></Field><Field label="Telegram"><div className="flex items-center gap-2 rounded-[8px] bg-white px-3 stroke"><span className="font-black text-[var(--muted-2)]">@</span><input value={tg} onChange={(event) => setTg(event.target.value.replace(/^@/, ""))} placeholder="username" className="w-full bg-transparent py-2.5 text-sm font-semibold outline-none" /></div></Field><Button className="w-full" onClick={() => { savePsyProfile({ name: name.trim(), tg: tg.trim() }); success(); onDone(); }}>Сохранить</Button></div>;
}

function ProfilePhoto({ photo, name, size }: { photo: string | null; name: string; size: "sm" | "lg" }) {
  const classes = size === "sm" ? "h-14 w-14 rounded-[10px] text-[20px]" : "h-[92px] w-[78px] rounded-[12px] text-[28px]";
