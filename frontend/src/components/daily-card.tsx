...
export function DailyCard({ moods, notes, onOpenMood, onSaveGood }: {
  ...
  return (
    <section className="rounded-[18px] bg-white p-4" style={{ border: "var(--bw-lg) solid var(--purple-edge)" }}>
...
function DailyRow({ icon, title, hint, done, onClick }: { icon: IconName; title: string; hint: string; done: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center gap-3 rounded-[13px] p-3 text-left transition-transform active:scale-[0.99]"
    ...
    <AnimatePresence>
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          />
          <motion.div
            className="relative w-full max-w-md rounded-t-[22px] bg-white px-5 pb-[calc(var(--safe-bottom)+20px)] pt-5"
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
          ...
          >
            ...
            <textarea
              ...
              className="mt-3 w-full resize-none rounded-[13px] p-3.5 text-[14px] font-semibold outline-none placeholder:font-normal placeholder:text-[var(--muted-2)]"
