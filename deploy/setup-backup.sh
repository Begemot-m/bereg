...
# || true обязателен: при отсутствии ключа grep возвращает 1, а из-за
# `set -e` вместе с `pipefail` это молча обрывало весь скрипт на первом же
# ещё не заданном параметре.
